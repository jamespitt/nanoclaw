...
**Docker mount syntax:** Both read-write and readonly mounts use `-v host:container` and `-v host:container:ro` respectively.
...
1. **Checks Docker is running** - Kills orphaned NanoClaw containers from previous runs
...
| "Claude Code process exited with code 1" | Docker not running | Start Docker: `sudo systemctl start docker` (Linux) or Docker Desktop (macOS) |
